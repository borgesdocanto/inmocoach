...
export default async function handler(
  ...
  try {
    ...
    // Búsqueda totalmente sin filtros
    const searchData = {
      filters: [],
    ...
    const solerProps = allProperties.filter((p: any) => 
      p.address?.toLowerCase().includes("soler 1175") ||
      p.address?.toLowerCase().includes("soler al 1175") ||
      p.address?.toLowerCase().includes("soler 1.175")
    );

    const mallorProps = allProperties.filter((p: any) =>
      p.address?.toLowerCase().includes("mallorca 1431") ||
      p.address?.toLowerCase().includes("mallorca al 1431") ||
      p.address?.toLowerCase().includes("mallorca 1.431") ||
      p.address?.toLowerCase().includes("mallorca piso 0") ||
      p.address?.toLowerCase().includes("mallorca departamento b")
    );

    res.status(200).json({
      success: true,
      total_all_properties: allProperties.length,
      soler_search: solerProps.length > 0 ? solerProps.map((p: any) => ({
      ...
      mallorca_search: mallorProps.length > 0 ? mallorProps.map((p: any) => ({
        reference_code: p.reference_code,
        address: p.address,
        status: p.status,
        branch_id: p.branch?.id,
