...
export default async function handler(
  ...
  try {
    ...
    // Buscar sin filtros para ver TODAS las propiedades reservadas
    const searchData = {
    ...
    if (!tokkoData.objects || tokkoData.objects.length === 0) {
      return res.status(404).json({ error: `No properties found with refCode ${refCode}` });
    }

    // Devolver TODAS las propiedades encontradas
    const results = tokkoData.objects.map((prop: any) => ({
      id: prop.id,
      reference_code: prop.reference_code,
      address: prop.address,
      branch: prop.branch?.name,
      producer: prop.producer?.name,
      producer_email: prop.producer?.email,
      key_agent_user: prop.internal_data?.key_agent_user?.name,
      key_agent_user_email: prop.internal_data?.key_agent_user?.email,
    }));

    res.status(200).json({
      success: true,
      refCode,
      total_found: tokkoData.objects.length,
      results,
    });
  } catch (error: any) {
