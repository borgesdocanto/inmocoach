...
export default async function handler(
  ...
  try {
    ...
    // Búsqueda sin filtro de branch para ver TODOS los datos
    const searchData = {
    ...
    // Devolver TODOS los campos sin filtrar
    res.status(200).json({
      message: "TODOS LOS DATOS DE LA PROPIEDAD (sin filtros)",
      refCode: property.reference_code,
      allData: property,
      keysSummary: Object.keys(property).sort(),
    });
  } catch (error: any) {
    console.error("[tokko-property-full] Error:", error);
