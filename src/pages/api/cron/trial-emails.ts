...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  for (const user of users) {
    ...
    if (daysUsed !== 3 && daysUsed !== 5) continue;
    ...
    const iacGoal = 15; // fallback — no bloqueamos el mail por esto

    const html = daysUsed === 3
      ? buildDay3Html(user.name || user.email, greenTotal, iacGoal)
      : buildDay5Html(user.name || user.email, greenTotal, iacGoal);

    const subject = daysUsed === 3
      ? `¿Cómo vas? · ${greenTotal} reuniones esta semana`
      : `Te quedan 2 días de prueba — no pierdas el acceso`;
  ...
  return res.status(200).json({ ok: true, day3, day5, failed });
}
